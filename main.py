...
def f_b12(phi, kappa, omega, XA, YA, ZA, XL, YL, ZL, f):
    ...
    deltaY = YA - YL
    deltaZ = ZA - ZL
    return (f / (q ** 2)) * (r * (m.cos(phi) * deltaX +m.sin(omega)*m.sin(phi)*deltaY-m.cos(omega)*m.sin(phi) * deltaZ) - q * (-m.sin(phi)*m.cos(kappa)*deltaX+m.sin(omega)*m.cos(phi)*m.cos(kappa) * deltaY -m.cos(omega)*m.cos(phi)*m.cos(kappa)* deltaZ))
...
def f_b22(phi, kappa, omega, XA, YA, ZA, XL, YL, ZL, f):
    ...
    deltaY = YA - YL
    deltaZ = ZA - ZL
    return (f / (q ** 2)) * (s * (m.cos(phi) * deltaX + m.sin(omega) * m.sin(phi) * deltaY - m.cos(omega) * m.sin(phi) * deltaZ) - q * (m.sin(phi) * m.cos(kappa) * deltaX - m.sin(omega) * m.cos(phi) * m.sin( kappa) * deltaY + m.cos(omega) * m.cos(phi) * m.sin(kappa) * deltaZ))
...
def Least_Squared(f, points):
    ...
    print('omega:{0},phi:{1}, kappa:{2}, XL:{3},YL:{4},ZL:{5}'.format((omega*180/m.pi),(phi*180/m.pi),(kappa*180/m.pi),XL,YL,ZL))
